...
- This is a mobile three-month anniversary experience for 宋智慧（昵称“三三”）.
- The three supplied landscape images in `public/assets/story/` are the source of truth for the visual language: blue-green night, reflected moonlight, warm streetlights, deep forest, visible grain, and quiet cinematic solitude.
- Preserve the supplied images as original assets; do not regenerate, stylize, or replace them.
- Keep the experience intimate and editorial. Avoid cute template styling, cartoon art, emoji, light-green stationery themes, generic card grids, and placeholder galleries.
...
- Let interaction reveal the relationship story instead of decorating it: include a tactile 92-day timeline with milestone feedback and a direct path from the final day to the keepsake.
